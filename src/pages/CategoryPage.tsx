...
export default function CategoryPage() {
  ...
  const { data: products = [], isLoading } = useProductsByCategoryFull(category!);
  const { data: brands } = useBrands(category!);

  const categoryLabel = CATEGORY_LABELS[category as ProductCategory];
  ...
  // 🔥 ordenar cada grupo por data (ou preço, etc)
  ...
  return (
    ...
    >
      <Helmet>
        <title>{categoryLabel} em Promoção | Ofertas e Achadinhos Baratos</title>
        <meta
          name="description"
          content={`Veja ofertas de ${categoryLabel.toLowerCase()} com preço baixo na Shopee, Amazon e Mercado Livre.`}
        />
      ...
      <div className="space-y-6">
        ...
        <section className="text-sm text-muted-foreground max-w-3xl mx-auto text-center pt-10">
          Encontre produtos da categoria {categoryLabel.toLowerCase()} em promoção e produtos baratos nos principais marketplaces.
          O Achadinhos LM reúne ofertas atualizadas para você pagar menos.
        </section>
